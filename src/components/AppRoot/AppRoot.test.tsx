...
describe('AppRoot', () => {
  ...
  it('has class .AppRoot--focus-visible on keyboard navigation and no class on mouse click', () => {
    render(<AppRoot data-testid="root" mode="embedded" />);
    const root = screen.getByTestId('root');

    // test keyboard nav and outline
    userEvent.tab();
    expect(root).toHaveClass('AppRoot--focus-visible');

    // test click and no outline
    userEvent.click(root);
    expect(root).not.toHaveClass('AppRoot--focus-visible');
  });
});
